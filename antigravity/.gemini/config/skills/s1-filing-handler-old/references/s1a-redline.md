...
errors on a large document, fall back to a **section-by-section** comparison: pull the key
sections of both filings with `edgar-filing-text` and compare only those (see the checklist
below) rather than dumping a raw diff.
